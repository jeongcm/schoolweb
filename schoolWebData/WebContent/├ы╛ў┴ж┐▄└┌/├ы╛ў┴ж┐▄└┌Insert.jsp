...
<%@ page import="취업제외자.취업제외자EditAction"%>
...
<script>
	window.opener.location.href = "취업제외자.jsp";
...
<%
	...
	try {
		...
		취업제외자EditAction edit = new 취업제외자EditAction();
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			int 계 = edit.계(진학자, 입대자, 취업불가능자, 외국인유학생, 건강보험직장가입제외대상, 입학당시기취업자);

			String sql = "insert into 취업제외자(년도,학과명,진학자,입대자,취업불가능자,외국인유학생,건강보험직장가입제외대상,입학당시기취업자,계,입력부서) values(?,?,?,?,?,?,?,?,?,?);";
