...
<%
	try {
		...
		ResultSet resultSet = statement.executeQuery(
				"select max(학과.년도) 년도,단과대학,학과.학과명,학문계열1,T점수 재학생T,(T점수*0.09) 재학생환산 from 학과현황 학과 left outer join 재학생현황 재학생 on 학과.학과명=재학생.학과명 group by 학과.학과명 order by 학과.연번 desc ;");
		ResultSet resultSet2 = statement2.executeQuery(
				"select 학과.학과명,T점수 신입생T,(T점수*0.09) 신입생환산 from 신입생현황 right outer join 학과현황 학과 on 신입생현황.학과명=학과.학과명 order by 학과.연번 desc ;");
		...
		XSSFSheet spreadsheet = workbook.createSheet("A그룹");
		...
		cell.setCellValue("신입생T점수");
		cell = row.createCell(7);
		cell.setCellValue("신입생환산점수");
		...
		cell.setCellValue("A그룹환산점수(취업률제외)");
		cell = row.createCell(17);
		cell.setCellValue("A그룹환산점수");
		...
		while (resultSet.next()&&resultSet2.next()&&resultSet3.next()&&resultSet4.next()&&resultSet5.next()&&resultSet6.next()) {
			...
			cell = row.createCell(4);
			cell.setCellValue(resultSet.getFloat("재학생T"));
			cell = row.createCell(5);
			cell.setCellValue((float) (Math.round(resultSet.getFloat("재학생환산") * 100) / 100.0));
			cell = row.createCell(6);
			cell.setCellValue(resultSet2.getFloat("신입생T"));
			cell = row.createCell(7);
			cell.setCellValue((float) (Math.round(resultSet2.getFloat("신입생환산") * 100) / 100.0));
			cell = row.createCell(8);
			...
			float 취업률제외환산 = (float) (Math.round(
					resultSet.getFloat("재학생환산") + resultSet2.getFloat("신입생환산") + resultSet6.getFloat("강의환산")
							+ resultSet4.getFloat("연구실적환산") + resultSet5.getFloat("연구비환산") * 100)
					/ 100.0);

			float 환산 = (float) (Math.round(resultSet.getFloat("재학생환산") + resultSet2.getFloat("신입생환산")
		...
		FileOutputStream fo = new FileOutputStream(
				new File(application.getRealPath("/tmpDownload/") + "A그룹.xlsx"));
		workbook.write(fo);
		fo.close();
		workbook.close();
	} catch (Exception e) {
...
<%
	...
	// 서버에 실제 저장된 파일명
	String filename = "A그룹.xlsx";

	// 실제 내보낼 파일명
	String orgfilename = "A그룹.xlsx";
