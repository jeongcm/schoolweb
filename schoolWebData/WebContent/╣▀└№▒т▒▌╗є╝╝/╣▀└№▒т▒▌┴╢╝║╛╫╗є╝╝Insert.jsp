...
<%@ page import="발전기금조성액상세자료.발전기금조성액상세자료Dao"%>
...
<script>
	window.opener.location.href = "발전기금조성액상세.jsp";
...
<%
	...
	try {
		...
		발전기금조성액상세자료EditAction edit = new 발전기금조성액상세자료EditAction();
		int 년도 = 0;
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			int 금액 = (int) row.getCell(3).getNumericCellValue();
			String 비고 = formatter.formatCellValue(row.getCell(4));

			String sql = "insert into 발전기금조성액상세자료(년도,학과명,구분,금액,비고,입력부서) values(?,?,?,?,?,?);";
			pstmt = (PreparedStatement) con.prepareStatement(sql);
			pstmt.setInt(1, 년도);
			pstmt.setString(2, 학과명);
			pstmt.setString(3, 구분);
			pstmt.setInt(4, 금액);
			pstmt.setString(5, 비고);
