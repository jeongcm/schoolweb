...
<%
	try {
		...
		ResultSet resultSet = statement.executeQuery("select 학과.단과대학,학과.학과명,개설.*,"
				+ "sum(if(구분='개설과목',학점,0)) as 전공과목," + "sum(if(구분='교양필수',학점,0)) as 교양필수과목 ,"
				+ "sum(if(구분='자유선택',학점,0)) as 자유선택과목  "
				+ "from 교원강의담당비율 교원 join 학과현황 학과 on 교원.년도=학과.년도 and 교원.학과명=학과.학과명"
				+ " join 개설강의담당비율 개설 on 학과.년도=개설.년도 and 학과.학과명=개설.학과명 " + "group by 학과.학과명 order by 연번 desc");
		XSSFWorkbook workbook = new XSSFWorkbook();
		XSSFSheet spreadsheet = workbook.createSheet("강의담당비율");
		...
		FileOutputStream fo = new FileOutputStream(
				new File(application.getRealPath("/tmpDownload/") + "강의담당비율.xlsx"));
	...
	String filename = "강의담당비율.xlsx";

	String orgfilename = "강의담당비율.xlsx";
