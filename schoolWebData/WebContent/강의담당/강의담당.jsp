...
function del(rowId){
	var 입력부서='<%=(String) session.getAttribute("id")%>';
	var row입력부서= $("#강의담당_list").jqGrid('getRowData', rowId).입력부서;
	if(입력부서==row입력부서){
		if (!insertCheck(입력부서)) {
	 		var 연번 = $("#강의담당_list").jqGrid('getRowData', rowId).연번;
  			 var button="&oper=del"
...
function editRow(id){
				var 입력부서='<%=(String) session.getAttribute("id")%>';
				var row입력부서 = $("#강의담당_list").jqGrid('getRowData', id).입력부서;
					
				if(입력부서==row입력부서){
					if (!insertCheck(입력부서)) {
							var lastid
							if (id && id !== lastid) {
								$('#강의담당_list').restoreRow(lastid);
									lastid = id;
								}
								$("#강의담당_list").editRow(id, true,null, reload);
						}
					}
				else{
					 swal(
...
function reload(response) {
				...
				if (result == 'fail') {
					...
					var id = $("#강의담당_list").jqGrid('getGridParam', "selrow");
					$('#강의담당_list').restoreRow(id);
					$.ajax({
						type : 'post',
						url : "../강의담당EditAction",
						beforeSend:function(){
			...
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[전임교원 강의담당 비율]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value="양식 다운로드" onclick="JavaScript:window.location='강의담당양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
							<input type=button value="엑셀 업로드" onclick="강의담당Up();" title="이미 작성된 엑셀을 업로드합니다">
							<input type=button value="엑셀 다운로드" onclick="JavaScript:window.location='강의비율Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다"></td>
						</tr>
						<tr>
							<td>년도</td>
							<td>단과대학</td>
					...
					$("#강의담당_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "연번", "년도", "학과명", "전공과목", "교양필수과목",
										"자유선택과목","입력부서","" ],
								colModel : [ {
									name : "연번",
									editable : true,
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../강의담당EditAction",
							});
		
					$("#강의담당_list").jqGrid("hideCol", [ "연번","입력부서" ]);
					$("#강의담당_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
