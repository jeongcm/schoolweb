...
<script>
	function del(rowId) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서= $("#취업제외자_list").jqGrid('getRowData', rowId).입력부서;
		
		if(입력부서==row입력부서){
		if (!insertCheck(입력부서)) {
					var 연번 = $("#취업제외자_list").jqGrid('getRowData', rowId).연번;
...
function editRow(id) {
	var 입력부서='<%=(String) session.getAttribute("id")%>';
	var row입력부서 = $("#취업제외자_list").jqGrid('getRowData', id).입력부서;
			if (입력부서 == row입력부서) {
					if (!insertCheck(입력부서)) {
			var lastid
			if (id && id !== lastid) {
				$('#취업제외자_list').restoreRow(lastid);
				lastid = id;
			}
			$("#취업제외자_list").editRow(id, true, null, reload);
				}
			} else {
				swal(
				    	'수정할 수 없습니다!',
...
function reload(response) {
	...
	if (result == 'fail') {
		...
		var id = $("#취업제외자_list").jqGrid('getGridParam', "selrow");

		$('#취업제외자_list').restoreRow(id);
			...
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[취업제외자]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value='취업률' onclick="window.open('../취업/취업률.jsp ', '_blank'); " />
							<input type=button value="양식 다운로드" onclick="JavaScript:window.location='취업제외자양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
							<input type=button value="엑셀 업로드" onclick="취업제외자Up();" title="이미 작성된 엑셀을 업로드합니다"></td>
						</tr>
						<tr>
							<td>년도</td>
							<td>단과대학</td>
						...
						<tr>
							...
							<td><input type='text' id="진학자" name="진학자"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type='text' id="입대자" name="입대자"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type='text' name="취업불가능자" id="취업불가능자"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type='text' name="건강보험직장가입제외대상" id="건강보험직장가입제외대상"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type='text' name="입학당시기취업자" id="입학당시기취업자"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type="text" name="외국인유학생" id="외국인유학생"
								onKeyup="if(event.keyCode ==13)save();"></td>		
					...
					$("#취업제외자_list").jqGrid(
							{
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../취업제외자EditAction",
							});
					$("#취업제외자_list").jqGrid("hideCol", [ "연번","입력부서" ]);
		
				
					$("#취업제외자_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
