...
<%
	try {
		...
		ResultSet resultSet = statement.executeQuery("select * from 취업제외자양식 ");
		XSSFWorkbook workbook = new XSSFWorkbook();
		XSSFSheet spreadsheet = workbook.createSheet("취업제외자 양식");
		...
		while (resultSet.next()) {
			...
			cell = row.createCell(1);
			cell.setCellValue(resultSet.getInt("학과명"));
			cell = row.createCell(2);
		...
		FileOutputStream fo = new FileOutputStream(
				new File(application.getRealPath("/tmpDownload/") + "취업제외자양식.xlsx"));
		workbook.write(fo);
		fo.close();
		workbook.close();
	} catch (Exception e) {
	...
	String filename = "취업제외자양식.xlsx";

	String orgfilename = "취업제외자양식.xlsx";
