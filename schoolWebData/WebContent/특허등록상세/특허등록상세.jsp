...
<script> 
			function save(){
				...
				if (!insertCheck(입력부서)) {
					...
					}  else if (document.insertData.대표발명자.value == "") {
						swal({
							  title: '대표발명자을 입력해주세요!',
							  type:'question',
							}).then(
							  function () {document.insertData.대표발명자.focus();})
					} else if (document.insertData.지식재산권.value == "") {
						swal({
							  title: '지식재산권을 입력해주세요!',
							  type:'question',
							}).then(
							  function () {document.insertData.지식재산권.focus();})
					}
				else{
							...
							if(msg=="fail"){
								...
								function (dismiss) {
									if(dismiss=='timer'){
										...
										document.insertData.대표발명자.value = "";
										document.insertData.지식재산권.value = "";
										document.insertData.정액기술료.value = "";
...
<script>
			function del(rowId){
				...
				var row입력부서= $("#특허등록상세_list").jqGrid('getRowData', rowId).입력부서;
				
				if(입력부서==row입력부서){
					if (!insertCheck(입력부서)) {
					
						var 연번 = $("#특허등록상세_list").jqGrid('getRowData', rowId).연번;
						var button="&oper=del"
...
function editRow(id){
		...
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서 = $("#특허등록상세_list").jqGrid('getRowData', id).입력부서;

		if (입력부서 == row입력부서) {
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#특허등록상세_list').restoreRow(lastid);
					lastid = id;
				}
				$("#특허등록상세_list").editRow(id, true, null, reload);
			}
		} else {
			swal(
			    	'수정할 수 없습니다!',
	...
	function reload(response) {
		...
		if (result == 'fail') {
			...
			var id = $("#특허등록상세_list").jqGrid('getGridParam', "selrow");
				$('#특허등록상세_list').restoreRow(id);
				}
		$("#특허등록상세_list").trigger("reloadGrid");
		$("#특허등록view_list").trigger("reloadGrid");
...
<body>
	...
	<div class="container">
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size="6">[상세자료(특허등록)]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value="양식 다운로드"
								onclick="JavaScript:window.location='특허등록상세양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
								<input type=button value="엑셀 업로드" onclick="특허등록상세Up();"
								title="이미 작성된 엑셀을 업로드합니다"> <input type=button
								value="엑셀 다운로드" onclick="JavaScript:window.location='../특허등록/특허등록Down.jsp';"
						...
						<tr>
							...
							<td>학과명</td>
							<td>대표발명자</td>
							<td>지식재산권 번호 및 노하우명</td>
							<td>입금 정액기술료(백만원)</td>
						</tr>
						<tr>
							<td><input type="text" name="년도" id="년도" size=3
							...
							<td><input type="text" name="대표발명자" id="대표발명자" size=2
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type="text" name="지식재산권" id="지식재산권" size=2
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type="text" name="정액기술료" id="정액기술료" size=2
								onKeyup="if(event.keyCode ==13)save();"></td>
			...
			<hr>
			<div id="pager"></div>
			<table id="특허등록상세_list"></table>

			<script type="text/javascript">
				$(function() {
					$("#특허등록상세_list").jqGrid(
							{
								url : "../특허등록상세ListAction",
								mtype : "post",
								datatype : "json",
								jsonReader : {
									repeatitems : false
								...
								rowNum : 20,
								rowList : [ 10, 15, 20 ],
								colNames : [ "년도", "학과명", "대표발명자",
										"지식재산권 번호 및 노하우명", "입금 정액기술료(백만원)",
										"연번", "입력부서", "" ],
								colModel : [ {
									name : "년도",
									align : "center",
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../특허등록상세EditAction"

							});

					$("#특허등록상세_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false
					...
					$("#특허등록상세_list").jqGrid("hideCol", [ "연번", "입력부서" ]);
				});
			</script>
					...
					$("#특허등록view_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 10, 15, 20 ],
								colNames : [ "단과대학","년도","학과명","전임교원수","국제","국내","특허이전료기술이전","특허등록및 기술이전 수입료","T점수"],
								colModel : [ {
									name : "단과대학",
									align : "center",
									edittype : "text",
