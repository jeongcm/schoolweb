...
public class 취업률Dao {
	...
	public int getCountRow() {
		String sql = "select count(*) from 취업률view";
		try {
	...
	public List<취업률> select취업률(int page, int perPageRow) {
		int beginRow = perPageRow * page - perPageRow;
		List<취업률> list = new ArrayList<취업률>();

		String sql = " select * from 취업률view limit ?,?;";
		try {
			...
			while (rs.next()) {
				...
				취업률 취업률 = new 취업률();
				취업률.set학문계열(rs.getString("학문계열1"));
				취업률.set년도(rs.getInt("년도"));
				취업률.set학과명(rs.getString("학과명"));
				취업률.set단과대학(rs.getString("단과대학"));
