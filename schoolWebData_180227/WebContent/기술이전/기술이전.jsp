...
<script>
			function del(rowId){
				...
				var row입력부서= $("#기술이전_list").jqGrid('getRowData', rowId).입력부서;
				
				if(입력부서==row입력부서){
					if (!insertCheck(입력부서)) {
					
						var 연번 = $("#기술이전_list").jqGrid('getRowData', rowId).연번;
						var button="&oper=del"
...
<script>
	function editRow(id){
		...
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서 = $("#기술이전_list").jqGrid('getRowData', id).입력부서;

		if (입력부서 == row입력부서) {
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#기술이전_list').restoreRow(lastid);
					lastid = id;
				}
				$("#기술이전_list").editRow(id, true, null, reload);
			}
		} else {
			swal(
			    	'수정할 수 없습니다!',
	...
	function reload(response) {
		...
		if (result == 'fail') {
			...
			var id = $("#기술이전_list").jqGrid('getGridParam', "selrow");

			$('#기술이전_list').restoreRow(id);
...
<body>
	...
	<div class="container">
			...
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size="6">[상세자료(기술이전)]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value="양식 다운로드"
								onclick="JavaScript:window.location='기술이전양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
								<input type=button value="엑셀 업로드" onclick="기술이전Up();"
		...
		$(function() {
			$("#기술이전_list").jqGrid(
					{
						...
						rowNum : 20,
						rowList : [ 10, 15, 20 ],
						colNames : [ "년도", "학과명", "대표발명자", "지식재산권 번호 및 노하우명",
								"입금 정액기술료(백만원)", "연번", "입력부서", "" ],
						colModel : [ {
							name : "년도",
							align : "center",
							editable : true,
						...
						onSelectRow : editRow,
						rownumbers : true,
						editurl : "../기술이전EditAction"

					});

			$("#기술이전_list").jqGrid("navGrid", "#pager", {
				search : false,
				edit : false,
				add : false,
				del : false
			...
			$("#기술이전_list").jqGrid("hideCol", [ "연번", "입력부서" ]);
		});
	</script>
			<br />
		<br />
