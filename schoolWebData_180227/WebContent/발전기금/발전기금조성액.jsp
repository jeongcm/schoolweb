...
<script>
	function del(rowId){
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서= $("#발전기금조성액_list").jqGrid('getRowData', rowId).입력부서;
		
		if(입력부서==row입력부서){
			if (!insertCheck(입력부서)) {
				var 연번 = $("#발전기금조성액_list").jqGrid('getRowData', rowId).연번;
...
<script>
	function editRow(id){
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서 = $("#발전기금조성액_list").jqGrid('getRowData', id).입력부서;

		if(입력부서==row입력부서){
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#발전기금조성액_list').restoreRow(lastid);
					lastid = id;
				}
				$("#발전기금조성액_list").editRow(id, true,null, reload);
	...
	function reload(response){
		...
		if (result == 'fail') {
			...
			var id = $("#발전기금조성액_list").jqGrid('getGridParam', "selrow");

			$('#발전기금조성액_list').restoreRow(id);
...
<body>
	...
	<div class="container">
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[발전기금조성액]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value="양식 다운로드" onclick="JavaScript:window.location='발전기금조성액양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
							<input type=button value="엑셀 업로드" onclick="발전기금조성액Up();" title="이미 작성된 엑셀을 업로드합니다">
							<input type=button value="엑셀 다운로드" onclick="JavaScript:window.location='발전기금조성액Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다"></td>
						</tr>
						<tr>
							<td>년도</td>
							<td>단과대학</td>
						...
						<tr>
							...
							<td><input type='text' id="지정기부금" name="지정기부금"
								onKeyup="if(event.keyCode ==13)save();"></td>
			...
			<hr>
			<div id="pager"></div>
			<table id="발전기금조성액_list"></table>
			<script type="text/javascript">
				$(function() {
					$("#발전기금조성액_list").jqGrid(
							{
								url : "../발전기금조성액ListAction",
								mtype : "post",
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../발전기금조성액EditAction",
							});
					
					$("#발전기금조성액_list").jqGrid("hideCol", [ "연번", "입력부서" ]);
		
					
					$("#발전기금조성액_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
			...
			<script type="text/javascript">
				$(function() {
					...
					$("#발전기금조성액view_list").jqGrid("navGrid", "#pager3", {
						search : false,
						edit : false,
						add : false,
						del : false,
