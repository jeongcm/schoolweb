...
<%
	try {
		...
		ResultSet resultSet = statement.executeQuery("select distinct 학과.단과대학,학과.학문계열1 학문계열,교원.일학기 전임교원수,연구.* "
				+ "from 학과현황 학과 join 교원현황 교원 on 학과.년도=교원.년도 and 학과.학과명=교원.학과명 join 연구비 연구 on 교원.학과명=연구.학과명 and 교원.년도=연구.년도"
				+ " where 교원.일학기 order by 연번 desc");
		XSSFWorkbook workbook = new XSSFWorkbook();
		
		XSSFSheet spreadsheet = workbook.createSheet("연구비");
		...
		FileOutputStream fo = new FileOutputStream(
				new File(application.getRealPath("/tmpDownload/") + "연구비.xlsx"));
	...
	String filename = "연구비.xlsx";

	String orgfilename = "연구비.xlsx";
