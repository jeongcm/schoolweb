...
<script>
	function del(rowId) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서= $("#취업현황_list").jqGrid('getRowData', rowId).입력부서;
		if(입력부서==row입력부서){
				if (!insertCheck(입력부서)) {
						var 연번 = $("#취업현황_list").jqGrid('getRowData', rowId).연번;
...
function editRow(id) {
	var 입력부서='<%=(String) session.getAttribute("id")%>';
	var row입력부서 = $("#취업현황_list").jqGrid('getRowData', id).입력부서;

	if (입력부서 == row입력부서) {
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#취업현황_list').restoreRow(lastid);
					lastid = id;
				}
				$("#취업현황_list").editRow(id, true, null, reload);
					}
			} else {
				 swal(
					    	'수정할 수 없습니다!',
...
function reload(response) {
	...
	if (result == 'fail') {
		...
		var id = $("#취업현황_list").jqGrid('getGridParam', "selrow");
		$('#취업현황_list').restoreRow(id);
				$.ajax({
					type : 'post',
					url : "../취업현황EditAction",
					beforeSend:function(){
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size="6">[취업현황]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value='취업률' onclick="window.open('취업률.jsp ', '_blank'); " />
							<input type=button value="양식 다운로드" onclick="JavaScript:window.location='취업현황양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
							<input type=button value="엑셀 업로드" onclick="취업현황Up();" title="이미 작성된 엑셀을 업로드합니다"></td>
						</tr>
						<tr>
							<td>년도</td>
							<td>단과대학</td>
						...
						<tr>
							...
							<td><input type='text' id="졸업자" name="졸업자"
								onKeyup="if(event.keyCode ==13)save();"></td>
							<td><input type='text' name="이차유지취업률" id="이차유지취업률"
								onKeyup="if(event.keyCode ==13)save();"></td>
			...
			<script type="text/javascript">
				$(function() {
					$("#취업현황_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "연번", "년도", "학과명","졸업자",
										"2차유지취업률", "취업률", "T점수", "입력부서","" ],
								colModel : [ {
									name : "연번",
									editable : true,
								}, {
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../취업현황EditAction",
							});
		
				
					$("#취업현황_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
					...
					$("#취업현황_list").jqGrid("hideCol", [ "연번","입력부서" ]);
