...
<script>
	...
	function save() {
		...
		}  else if (document.insertData._5대계열.value == "5대계열") {
			swal({
				  title: '5대계열을 입력해주세요!',
				  type:'question',
				}).then(
				  function () {document.insertData._5대계열.focus();})
			}  else if (document.insertData.학문계열1.value == "학문계열") {
				swal({
					  title: '학문계열을 선택해주세요!',
					  type:'question',
					}).then(
					  function () {document.insertData.학문계열1.focus();})
			} else {
				...
				$("#학과현황_list").trigger("reloadGrid");
				document.insertData.학과명.value = "";
				document.insertData._5대계열.value = "5대계열";
				document.insertData.학문계열1.value = "학문계열";
				document.insertData.신설연도.value = "";
...
function del(rowId) {
	var 입력부서='<%=(String) session.getAttribute("id")%>';
	var row입력부서=$("#학과현황_list").jqGrid('getRowData', rowId).입력부서;
	if(입력부서==row입력부서){
		if (!insertCheck(입력부서)) { 
			var 연번 = $("#학과현황_list").jqGrid('getRowData', rowId).연번;
					var button = '&oper=del';
							$.ajax({
									url : "../학과현황EditAction",
									type : "post",
...
<Script>
	function editRow(id) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서= $("#학과현황_list").jqGrid('getRowData', id).입력부서;
		if(입력부서==row입력부서){
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#학과현황_list').restoreRow(lastid);
					lastid = id;
				}
				$("#학과현황_list").editRow(id, true,null, reload);
	...
	function reload(response) {
		...
		if (result == 'fail') {
			...
			var id = $("#학과현황_list").jqGrid('getGridParam',"selrow");
		}	
		swal({type:'success',
			  title: '요청완료!',
			  timer:500}).then(function () {},
			...
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size="6">[학과현황]</font></th>
						</tr>
						<tr>
							<td colspan="20">
								<input type=button value="양식 다운로드" onclick="onJavaScript:window.location='학과현황양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다"> 
								<input type=button value="엑셀 업로드" onclick="학과현황Up();" title="이미 작성된 엑셀을 업로드합니다">
								 <input type=button value="엑셀 다운로드" onclick="onJavaScript:window.location='학과현황Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다">
							</td>
						</tr>
						<tr>
							<td>년도</td>
						...
						<tr>
							...
							<td>
								<input type="text" name="학과명" id="학과명" onKeyup="if(event.keyCode ==13)save();">
							</td>
							<td>
								<select name="_5대계열">
									<option>5대계열</option>
							...
							<td>
								<input type='text' name="신설연도" id="신설연도" onKeyup="if(event.keyCode ==13)save();">
							</td>
							<td>
							...
							.jqGrid(
									{
										...
										colModel : [
												...
												},
												{
													...
													edittype : "select",
													editoptions : {
														value : "융합기술대학:융합기술대학;건설교통대학:건설교통대학;공과대학:공과대학;인문사회대학:인문사회대학;보건생명대학:보건생명대학;철도대학:철도대학"
													},
													sortable : false
												},
												{
												...
												},
												{
													...
													edittype : "select",
													editoptions : {
														value : ":;공학:공학;인문사회:인문사회;예체능:예체능;자연과학:자연과학"
													},
													sortable : false
												},
												{
										...
										editurl : "../학과현황EditAction",
									});
					$("#학과현황_list").jqGrid("hideCol", [ "연번","입력부서" ]);
					$("#학과현황_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
