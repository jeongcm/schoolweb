...
<script>
	function save() {
		...
		if (!insertCheck(입력부서)) {
						...
						} else if (document.insertData.입학자수.value == "") {
							swal({
								  title: '정원내 입학자 수를 입력해주세요!',
								  type:'question',
								}).then(
								  function () {document.insertData.입학자수.focus();})
							} else if (document.insertData.모집인원.value == "") {
								swal({
									  title: '정원내 모집인원을 입력해주세요!',
									  type:'question',
									}).then(
									  function () {document.insertData.모집인원.focus();})
							} else {
										...
										.ajax({
											...
											success : function(msg) {
												if (msg == 'fail') {
													...
													function (dismiss) {
														if(dismiss=='timer'){
															$("#신입생현황_list").trigger("reloadGrid");
															$("#신입생현황").trigger("reloadGrid");

															document.insertData.학과명.value = "학과명";
															document.insertData.입학자수.value = "";
															document.insertData.모집인원.value = "";
...
<script>
	function del(rowId) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서=$("#신입생현황_list").jqGrid('getRowData', rowId).입력부서;
		if(입력부서==row입력부서){
			if (!insertCheck(입력부서)) { 
				var 연번 = $("#신입생현황_list").jqGrid('getRowData', rowId).연번;
						var button = '&oper=del';
								$.ajax({
										url : "../신입생현황EditAction",
										type : "post",
...
<script>
	function editRow(id) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row입력부서= $("#신입생현황_list").jqGrid('getRowData', id).입력부서;
		if(입력부서==row입력부서){
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#신입생현황_list').restoreRow(lastid);
					lastid = id;
				}
				$("#신입생현황_list").editRow(id, true,null, reload);
			}
		} else {
			swal(
			    	'수정할 수 없습니다!',
	...
	function reload(response) {
		...
		if (result == 'fail') {
			...
			var id = $("#신입생현황_list").jqGrid('getGridParam',"selrow");
			$('#신입생현황_list').restoreRow(id);
				$.ajax({
					type : 'post',
					url : "../신입생현황EditAction",
					beforeSend:function(){
...
<body>
	...
	<div class="container">
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[정원내 신입생 충원율]</font></th>
						</tr>
						<tr>
							<td colspan="20">
								<input type=button value="양식 다운로드" onclick="onJavaScript:window.location='신입생현황양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다"> 
								<input type=button value="엑셀 업로드" onclick="신입생현황Up();" title="이미 작성된 엑셀을 업로드합니다"> 
								<input type=button value="엑셀 다운로드" onclick="onJavaScript:window.location='신입생현황Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다">
							</td>
						</tr>
						<tr>
							<td>년도</td>
						...
						<tr>
							...
							<td>
								<input type='text' id="입학자수" name="입학자수" onKeyup="if(event.keyCode ==13)save();">
							</td>
							<td>
								<input type='text' id="모집인원" name="모집인원" onKeyup="if(event.keyCode ==13)save();">
							</td>
							<td>
					...
					$("#신입생현황_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "연번", "년도", "학과명", "정원내 입학자수", "정원내 모집인원","입력부서", "" ],
								colModel : [ {
									name : "연번",
									editable : true,
								}, {
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../신입생현황EditAction",
							});
					$("#신입생현황_list").jqGrid("hideCol", [ "연번","입력부서" ]);
		
					
					$("#신입생현황_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
			...
			<br /> <br />
			<div id="pager2"></div>
			<table id="신입생현황"></table>
			<script type="text/javascript">
				$(function() {
					$("#신입생현황").jqGrid(
							{
								url : "../신입생현황ViewListAction",
								mtype : "post",
								datatype : "json",
								caption : "신입생현황",
								height : "auto",
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "년도", "단과대학", "학과명", "정원내 입학자수",
										"정원내 모집인원", "정원내 신입생 충원율", "T점수" ],
								colModel : [ {
									name : "년도",
									align : "center",
									sortable : false,
