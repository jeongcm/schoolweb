								...
								if (msg == 'fail') {
									...
									function (dismiss) {
										if(dismiss=='timer'){
											...
											$("#연구비_list").trigger("reloadGrid");
											$("#연구비view_list").trigger("reloadGrid");
...
<script>
	function del(rowId) {
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row부서=$("#연구비_list").jqGrid('getRowData', rowId).입력부서;
			if(입력부서==row부서){
				if (!insertCheck(입력부서)) { 
					연번 = $("#연구비_list").jqGrid('getRowData', rowId).연번;
					button="&oper=del"
					$.ajax({
							url : "../연구비EditAction",
							type : "post",
...
<script>
	function editRow(id){
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row부서= $("#연구비_list").jqGrid('getRowData', id).입력부서;
		if(입력부서==row부서){
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#연구비_list').restoreRow(lastid);
					lastid = id;
				}
				$("#연구비_list").editRow(id, true,null, reload);
			}
		} else {
			swal(
			    	'수정할 수 없습니다!',
	...
	function reload(response){
			...
			if (result == 'fail') {
				...
				var id = $("#연구비_list").jqGrid('getGridParam', "selrow");
				$('#연구비_list').restoreRow(id);
...
<body>
	...
	<div class="container">
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[전임교원 1인당 교외연구비]</font></th>
						</tr>
						<tr>
							<td colspan="20">
								<input type=button value="양식 다운로드" onclick="onJavaScript:window.location='연구비양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다"> <input type=button value="엑셀 업로드" onclick="연구비Up();" title="이미 작성된 엑셀을 업로드합니다"> <input type=button value="엑셀 다운로드" onclick="onJavaScript:window.location='연구비Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다">
							</td>
						</tr>
						<tr>
							<td>년도</td>
					...
					$("#연구비_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "연번", "년도", "학과명", "중앙정부", "지자체", "민간",
										"외국", "입력부서","" ],
								colModel : [ {
									name : "연번",
									editable : true,
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../연구비EditAction",
							});
		
				
					$("#연구비_list").jqGrid("hideCol", [ "연번","입력부서" ]);
		
					$("#연구비_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false,
			...
			<br /> <br />
			<div id="pager2"></div>
			<table id="연구비view_list"></table>
			<script type="text/javascript">
				$(function() {
					$("#연구비view_list")
							.jqGrid(
									{
										url : "../연구비viewListAction",
										mtype : "post",
					...
					$("#연구비view_list").jqGrid("navGrid", "#pager2", {
						search : false,
						edit : false,
						add : false,
						del : false,
