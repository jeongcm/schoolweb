				...
				$.ajax({
					...
					success : function(msg) {
						if (msg == 'fail') {
							...
							function (dismiss) {
								if(dismiss=='timer'){	
								$("#교외장학금_list").trigger("reloadGrid");
						        $("#교외장학금view_list").trigger("reloadGrid");
								
						        
						    document.insertData.학과명.value = "학과명";
								document.insertData.교외장학금.value= "";
...
<script>
	function del(rowId){
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row부서= $("#교외장학금_list").jqGrid('getRowData', rowId).입력부서;
		
		if(입력부서==row부서){
			if (!insertCheck(입력부서)) {
				var 연번 = $("#교외장학금_list").jqGrid('getRowData', rowId).연번;
...
<script>
	function editRow(id){
		var 입력부서='<%=(String) session.getAttribute("id")%>';
		var row부서 = $("#교외장학금_list").jqGrid('getRowData', id).입력부서;

		if(입력부서==row부서){
			if (!insertCheck(입력부서)) {
				var lastid
				if (id && id !== lastid) {
					$('#교외장학금_list').restoreRow(lastid);
					lastid = id;
				}
				$("#교외장학금_list").editRow(id, true,null, reload);
			}
		} else {
			swal(
			    	'수정할 수 없습니다!',
	...
	function reload(response){
		...
		if (result == 'fail') {
			...
			var id = $("#교외장학금_list").jqGrid('getGridParam', "selrow");
			$('#교외장학금_list').restoreRow(id);
...
<body>
	...
	<div class="container">
		...
		<div class="contents">
			<div class=top>
				<form id="insertData" onsubmit="return false" name="insertData">
					<table style="text-align: center;">
						<tr>
							<th colspan="20"><font size='6'>[재학생 1인당 교외장학금]</font></th>
						</tr>
						<tr>
							<td colspan="20"><input type=button value="양식 다운로드" onclick="onJavaScript:window.location='교외장학금양식Down.jsp';" title="엑셀을 업로드 하기위해 필요한 양식을 다운 받습니다">
							<input type=button value="엑셀 업로드" onclick="교외장학금Up();" 	title="이미 작성된 엑셀을 업로드합니다">
							<input type=button value="엑셀 다운로드" onclick="onJavaScript:window.location='교외장학금Down.jsp';" title="입력되어있는 데이터를 엑셀 형식으로 다운 받습니다"></td>
						</tr>
						<tr>
							<td>년도</td>
							<td>단과대학</td>
						...
						<tr>
							...
							<td><input type='text' id="교외장학금" name="교외장학금"
								onKeyup="if(event.keyCode ==13)save();"></td>
					...
					$("#교외장학금_list").jqGrid(
							{
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "년도","학과명","교외장학금(천원)","연번","입력부서","" ],
								colModel : [ {
									name : "년도",
									align : "center",
									editable : true,
								...
								onSelectRow : editRow,
								rownumbers : true,
								editurl : "../교외장학금EditAction",
							});
					
					$("#교외장학금_list").jqGrid("hideCol", [ "연번" ,"입력부서"]);
					
			
					
					$("#교외장학금_list").jqGrid("navGrid", "#pager", {
						search : false,
						edit : false,
						add : false,
						del : false
			...
			<br/>
			<div id="pager1"></div>
			<table id="교외장학금view_list"></table>
			<script type="text/javascript">
				$(function() {
					$("#교외장학금view_list").jqGrid(
							{
								url : "../교외장학금viewListAction",
								mtype : "post",
								datatype : "json",
								caption : "교외장학금",
								height : "auto",
								...
								rowNum : 20,
								rowList : [ 20, 50, 100 ],
								colNames : [ "년도","단과대학","학과명","평균","1학기","2학기","교외장학금","1인당교외장학금","T점수" ],
								colModel : [ {
									name : "년도",
									align : "center",
									edittype : "text",
								...
								rownumbers : true,
								hiddengrid : true,
								editurl : "../교외장학금viewEditAction"
							});
					
					$("#교외장학금view_list").jqGrid("navGrid", "#pager1", {
						search : false,
						edit : false,
						add : false,
						del : false,
