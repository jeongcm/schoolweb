...
@WebServlet("/전임교원확보율ListAction")
public class 전임교원확보율ListAction extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<전임교원확보율> list = dao.select전임교원확보율(page, perPageRow);
		int records = dao.getCountRow();
		int total = (int) Math.ceil((double) records / (double) perPageRow);

		전임교원확보율Json 전임교원확보율Json = new 전임교원확보율Json();
